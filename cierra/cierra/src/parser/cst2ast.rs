...
pub fn parse_main(ctx: &MainContextAll) -> Taurus {
    let defs = ctx.def_all().into_iter().flat_map(|ctx| parse_def(&ctx)).collect();
    Taurus { defs }
}
...
fn parse_func(ctx: &FuncDefContextAll) -> FuncDef {
    let name = ctx.retVar().unwrap().IDENT().unwrap().get_text().into();
    let contract = parse_contract(&ctx.funcContract().unwrap());
    let ret_ty = parse_ret_ty(&ctx.retVar().unwrap().retTy().unwrap());
    let vars = ctx.paraVar_all().into_iter().map(|ctx| parse_para_var(&ctx)).collect();
    let decl_stmts = ctx.declStmt_all().into_iter().map(|ctx| parse_decl_stmt(&ctx)).collect();
    FuncDef { name, contract, ret_ty, vars, decl_stmts }
}

fn parse_struct(ctx: &StructDefContextAll) -> StructDef {
    let name = ctx.IDENT().unwrap().get_text().into();
    let members = ctx.paraVar_all().into_iter().map(|ctx| parse_para_var(&ctx)).collect();
    StructDef { name, members }
}

fn parse_pred_def(ctx: &PredDefContextAll) -> PredDef {
    let name = ctx.IDENT().unwrap().get_text().into();
    let vars = ctx.logicParaVar_all().into_iter().map(|ctx| parse_logic_para_var(&ctx)).collect();
...
fn parse_pred(ctx: &PredContextAll) -> Pred {
    match ctx {
        ...
        PredContextAll::LengthPredContext(ctx) => Pred::Length {
            base: ctx.IDENT().unwrap().get_text().into(),
            start: ctx.INT_CONSTANT().unwrap().get_text().parse().unwrap(),
            end: parse_arith_term(&ctx.arithTerm().unwrap()),
        },
        PredContextAll::QuantiPredContext(ctx) => Pred::Quant(
            parse_quantifier(&ctx.quantifier().unwrap()),
            ctx.binder_all().into_iter().flat_map(|ctx| parse_binder(&ctx)).collect(),
            Box::new(parse_pred(&ctx.pred().unwrap())),
        ),
        ...
        PredContextAll::CallPredContext(ctx) => Pred::Call(
            ctx.IDENT().unwrap().get_text().into(),
            ctx.term_all().into_iter().map(|ctx| parse_term(&ctx)).collect(),
        ),
        ...
        PredContextAll::CmpPredContext(ctx) => {
            let mut terms = ctx.arithTerm_all().into_iter().map(|ctx| parse_arith_term(&ctx));
            let ops = ctx.cmpOp_all().into_iter().map(|ctx| parse_cmp_op(&ctx));
            let initial = terms.next().unwrap();
            Pred::Cmp(ops.zip(terms).fold(PredCmp::ArithTerm(initial), |acc, (op, term)| {
...
fn parse_arith_term(ctx: &ArithTermContextAll) -> ArithTerm {
    match ctx {
        ...
        ArithTermContextAll::IdentTermContext(ctx) => {
            ArithTerm::Var(ctx.IDENT().unwrap().get_text().into())
        }
        ArithTermContextAll::UnaryTermContext(ctx) => ArithTerm::Unary(
            parse_unary_op(&ctx.unaryOp().unwrap()),
            Box::new(parse_arith_term(&ctx.arithTerm().unwrap())),
        ),
        ArithTermContextAll::ArrAccessTermContext(ctx) => ArithTerm::Read(
            Box::new(parse_arith_term(&ctx.arithTerm(0).unwrap())),
            Box::new(parse_arith_term(&ctx.arithTerm(1).unwrap())),
        ),
        ArithTermContextAll::ResTermContext(_) => ArithTerm::Res,
        ArithTermContextAll::ArrUpdTermContext(ctx) => ArithTerm::Store(
            Box::new(parse_arith_term(&ctx.arithTerm(0).unwrap())),
            Box::new(parse_arith_term(&ctx.arithTerm(1).unwrap())),
            Box::new(parse_arith_term(&ctx.arithTerm(2).unwrap())),
        ),
...
fn parse_contract(ctx: &FuncContractContextAll) -> Contract {
    let requires =
        ctx.requiresClause_all().into_iter().map(|ctx| parse_pred(&ctx.pred().unwrap())).collect();
    let ensures =
        ctx.ensuresClause_all().into_iter().map(|ctx| parse_pred(&ctx.pred().unwrap())).collect();
...
fn parse_ret_ty(ctx: &RetTyContextAll) -> RetTy {
    match ctx {
        ...
        RetTyContextAll::StructRetTyContext(ctx) => {
            RetTy::Struct(ctx.IDENT().unwrap().get_text().into())
        }
        RetTyContextAll::Error(_) => unreachable!(),
    }
}

fn parse_para_var(ctx: &ParaVarContextAll) -> (concrete::ParaTy, Symbol) {
    match ctx {
        ...
        ParaVarContextAll::StructParaVarContext(ctx) => (
            concrete::ParaTy::Struct(ctx.IDENT(0).unwrap().get_text().into()),
            ctx.IDENT(1).unwrap().get_text().into(),
        ),
        ParaVarContextAll::Error(_) => unreachable!(),
    }
}

fn parse_logic_para_var(ctx: &LogicParaVarContextAll) -> (logic::ParaTy, Symbol) {
    match ctx {
        ...
        LogicParaVarContextAll::StructLogicParaVarContext(ctx) => (
            logic::ParaTy::Struct(ctx.IDENT(0).unwrap().get_text().into()),
            ctx.IDENT(1).unwrap().get_text().into(),
        ),
...
fn parse_local_var(ctx: &LocalVarContextAll) -> (LocalTy, Symbol) {
    match ctx {
        ...
        LocalVarContextAll::StructLocalVarContext(ctx) => (
            LocalTy::Struct(ctx.IDENT(0).unwrap().get_text().into()),
            ctx.IDENT(1).unwrap().get_text().into(),
        ),
...
fn parse_binder(ctx: &BinderContextAll) -> Vec<Binder> {
    ...
    ctx.IDENT_all().into_iter().map(|ident| Binder(ty, ident.get_text().into())).collect()
}
...
fn parse_expr(ctx: &ExprContextAll) -> Expr {
    match ctx {
        ...
        ExprContextAll::IdentExprContext(ctx) => Expr::Var(ctx.IDENT().unwrap().get_text().into()),
        ...
        ExprContextAll::ArrAccessExprContext(ctx) => Expr::Read(
            Box::new(parse_expr(&ctx.expr(0).unwrap())),
            Box::new(parse_expr(&ctx.expr(1).unwrap())),
        ),
        ...
        ExprContextAll::CallExprContext(ctx) => Expr::Call(
            ctx.IDENT().unwrap().get_text().into(),
            ctx.expr_all().into_iter().map(|ctx| parse_expr(&ctx)).collect(),
        ),
...
fn parse_stmt(ctx: &StmtContextAll) -> Stmt {
    match ctx {
        StmtContextAll::IfStmtContext(ctx) => Stmt::If(
            ...
            Box::new(parse_stmt(&ctx.stmt(0).unwrap())),
            Box::new(parse_stmt(&ctx.stmt(1).unwrap())),
        ),
        StmtContextAll::ExprStmtContext(ctx) => Stmt::Expr(parse_expr(&ctx.expr().unwrap())),
        StmtContextAll::WhileStmtContext(ctx) => Stmt::While(
            parse_loop_annot(&ctx.loopAnnot().unwrap()),
            parse_expr(&ctx.expr().unwrap()),
            Box::new(parse_stmt(&ctx.stmt().unwrap())),
        ),
        ...
        StmtContextAll::BlockStmtContext(ctx) => {
            Stmt::Block(ctx.declStmt_all().into_iter().map(|ctx| parse_decl_stmt(&ctx)).collect())
        }
        ...
        StmtContextAll::ForStmtContext(ctx) => Stmt::For(
            parse_loop_annot(&ctx.loopAnnot().unwrap()),
            parse_for_init(&ctx.forInit().unwrap()),
            ctx.expr().map(|ctx| parse_expr(&ctx)),
            ctx.stmt(0).map(|ctx| Box::new(parse_stmt(&ctx))),
            Box::new(parse_stmt(&ctx.stmt(1).unwrap())),
        ),
...
fn parse_assign(ctx: &AssignContextAll) -> Assign {
    match ctx {
        AssignContextAll::MemAssignContext(ctx) => Assign::Mem(
            ctx.IDENT(0).unwrap().get_text().into(),
            ctx.IDENT(1).unwrap().get_text().into(),
            parse_expr(&ctx.expr().unwrap()),
        ),
        AssignContextAll::VarAssignContext(ctx) => {
            Assign::Var(ctx.IDENT().unwrap().get_text().into(), parse_expr(&ctx.expr().unwrap()))
        }
        AssignContextAll::SubAssignContext(ctx) => Assign::Sub(
            ctx.IDENT().unwrap().get_text().into(),
            parse_expr(&ctx.expr(0).unwrap()),
            parse_expr(&ctx.expr(1).unwrap()),
        ),
...
fn parse_loop_annot(ctx: &LoopAnnotContextAll) -> LoopAnnot {
    LoopAnnot {
        invariants: ctx.pred_all().into_iter().map(|ctx| parse_pred(&ctx)).collect(),
        variants: ctx.arithTerm_all().into_iter().map(|ctx| parse_arith_term(&ctx)).collect(),
    }
}
