...
std::string LowIRGenerator::GenDecl(DeclIR &decl, std::string &code) {
    if (currentFunc.empty()) {
        ...
        if (decl.getType() == VarType::array_t) {
            ...
            varStack[decl.getName()] = StackVar(funcStack[currentFunc], funcStack[currentFunc], currentFunc, VarType::var_t);
...
std::string LowIRGenerator::GenInit(InitIR &init, std::string &code) {
    if (currentFunc.empty()) {
        ...
        code += "\tstore s" + std::to_string(reg_num) + " "
                + std::to_string(
                varStack[init.getName()].pos_min + (init.getType() == VarType::array_t ? init.getPos() : 0)) + "\n";
        //reg_num--;
    }
    return {};
}
...
std::string LowIRGenerator::GenFuncCall(FuncCallIR &funcCall, std::string &code) {
    ...
    if (lval.getName()[0] == 'T') {
        if (globalVars.find(lval.getName()) != globalVars.end()) {
            ...
            if (lval.getType() == VarType::var_t) {
                ...
                if (dynamic_cast<RightValIR *>(lval.getPos().get())->getType() == IRToken::NUMBER_IR) {
...
std::string LowIRGenerator::GenUnaryExp(UnaryExpIR &unary, std::string &code) {
    if (dynamic_cast<RightValIR*>(unary.getExp().get())->getType() == IRToken::SYMBOL_IR) {
        code += "\tload " +
...
std::string LowIRGenerator::GenBinaryExp(BinaryExpIR &binary, std::string &code) {
    std::string lhs = binary.getLHS()->Generate(*this, code);
    std::string rhs = binary.getRHS()->Generate(*this, code);
    std::set<std::string> logicOp{">=", "<=", "==", "!=", ">", "<"};
    if (dynamic_cast<RightValIR*>(binary.getRHS().get())->getType() == IRToken::NUMBER_IR
    && logicOp.find(op2char(binary.getOp())) != logicOp.end()) {
        code += "\ts" + std::to_string(reg_num) + " = " + rhs + "\n";
        rhs = "s" + std::to_string(reg_num);
        reg_num++;
    }
    if (dynamic_cast<RightValIR*>(binary.getLHS().get())->getType() == IRToken::NUMBER_IR
    && logicOp.find(op2char(binary.getOp())) != logicOp.end()) {
    ...
    if (dynamic_cast<RightValIR*>(binary.getLHS().get())->getType() == IRToken::NUMBER_IR
    && dynamic_cast<RightValIR*>(binary.getRHS().get())->getType() == IRToken::SYMBOL_IR) {
        code += "\ts" + std::to_string(reg_num) + " = " + lhs + "\n";
        lhs = "s" + std::to_string(reg_num);
        reg_num++;
    }
...
std::string LowIRGenerator::GenRightVal(RightValIR &rightval, std::string &code) {
...
std::string LowIRGenerator::GenParamList(ParamListIR &params, std::string &code) {
...
std::string LowIRGenerator::GenStatements(StatementsIR &stmts, std::string &code) {
...
std::string StatementsIR::Generate(LowIRGenerator &generator, std::string &code) {
    return generator.GenStatements(*this, code);
}

std::string BinaryExpIR::Generate(LowIRGenerator &generator, std::string &code) {
...
std::string ParamListIR::Generate(LowIRGenerator &generator, std::string &code) {
