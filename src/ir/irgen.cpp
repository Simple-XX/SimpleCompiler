...
void IRGenerator::GenerateValue(const std::string &varName, int &idx, int indx, InitValAST *init, std::vector<int> dim,
...
std::string IRGenerator::GenVarDef(VarDefAST &varDef, std::string &code) {
    ...
    if (varDef.getInitVal()) {
        ...
        if (dynamic_cast<ProcessedIdAST *>(varDef.getVar().get())->getType() == VarType::var_t) {
