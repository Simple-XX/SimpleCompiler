...
//
// irparser.cpp for Simple-XX/SimpleCompiler.
...
void IRParser::NextIRToken() {
    current = lexer.NextIRToken();
}
...
IRPtr IRParser::ParseDecl() {
    ...
    if (current == IRToken::NUMBER_IR) {
        ...
        name = lexer.getName();
        NextIRToken();
        return std::make_unique<DeclIR>(VarType::array_t, size, name, lexer.getLineno() - 1);
    } else {
        name = lexer.getName();
        NextIRToken();
        return std::make_unique<DeclIR>(VarType::var_t, 0, name, lexer.getLineno() - 1);
    }
}
...
IRPtr IRParser::ParseFuncDef() {
    if (current != IRToken::SYMBOL_IR || lexer.getName()[0] != 'f' || lexer.getName()[1] != '_') {
        exit(208);
    }
...
IRPtr IRParser::ParseAssign() {
    ...
    if (current == IRToken::OP_IR) {
        ...
        if (current == IRToken::LSB_IR) {
            ...
            NextIRToken(); 
            rhs = std::make_unique<LValIR>(VarType::array_t, name, lexer.getLineno() - 1, std::move(rValue));
            return std::make_unique<AssignIR>(std::move(lhs), std::move(rhs), lexer.getLineno() - 1);
        } else {
            IRPtr rValue_1 = std::make_unique<RightValIR>(IRToken::SYMBOL_IR, name, lexer.getLineno());
            if (current == IRToken::OP_IR || current == IRToken::LOGICOP_IR) {
                if (dynamic_cast<LValIR *>(lhs.get())->getType() == VarType::array_t) {
                    exit(223);
                }
...
IRPtr IRParser::ParseLVal() {
    ...
    if (current == IRToken::LSB_IR) {
        ...
        NextIRToken();
        return std::make_unique<LValIR>(VarType::array_t, name, lexer.getLineno(), std::move(rValue));
    } else {
